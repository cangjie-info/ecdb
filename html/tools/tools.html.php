...
<p>This page outlines all of the tools, software and other
components that are currently being used in developing the ECDB
project. The aim to provide a sufficiently complete description so
...
<p>The original motivation for using Linux was the greater
availability and ease of use of certain open source tools, compared
with MS Windows. A second motivation was to ensure that as ECDB
developed, it would remain replicatable independently of the usual
hardware or operating system choices.</p>
<h3>Microsoft Windows 7</h3>
<p>Although ECDB development began on Linux, it was important that
everything be portable onto more popular desktop operating systems.
If everything is portable between Linux and MS Windows, I am
assuming that portability to the Mac OSs will not be too difficult
either.</p>
<h2>Web hosting</h2>
...
web-interfaces to the database, are handcoded using HTML, CSS, and
PHP. For the most part, nothing more complex that what could be
found in an introductory textbook has been used. Anything more
complex than that will be documented in these pages.</p>
<h2>MySQL</h2>
<ul>
  <li>MySQL &gt; ver 5.5</li>
...
<p>For local development versions of the web documentation and PHP
interfaces to the database, a standard Wamp (Windows) or Lamp
(Ubuntu) stack is used with minimal modification.
<p>
  <h2>Qt, C++</h2>
  <ul>
    <li>Qt 4</li>
  ...
  using the msi installer. Visual C++ is one (cost-free) option for
  a compiler on Windows, but I think MinGW is sufficient for all
  purposes.</p>
  <p>One complication on Windows. Qt needs a plugin to access MySQL
  servers. This seems to be absent on the standard Windows
  installation, and so needs to be compiled. The compilation
  requires the MySQL library file 
  <code>libmysql.dll</code>and possibly other MySQL development
  ...
  <ul>
    ...
    plugin.</li>
    <li>Confirm that 
    <code>libmysql.lib</code>is present.</li>
    <li>Follow Seppe's instructions, modifying paths and file names
    as necessary.</li>
    <li>Delete the unzipped mysql files as they are no longer
    necessary. (Don't delete the MySQL files that cam with the Wamp
    package.)</li>
  </ul>
  <h2>Python</h2>
  <ul>
  ...
  <p>For quicker solutions, particularly data wrangling, Python is
  used.</p>
  <h2>Git</h2>
  ...
  <ul>
    <li>Add 
    <code>.jpg</code>file name extension to all files in current
    directory: 
    <code>find . -type f -exec mv {} {}.jpg ';'</code></li>
    <li>Indent and wrap lines of html file, without adding or
    removing any tags: 
